...
class HiscoreRecord(Base):
    ...
    overall_level: Mapped[Optional[int]] = mapped_column(
        Integer,
        nullable=True,
        doc="Total level across all skills"
    )
    
    overall_experience: Mapped[Optional[int]] = mapped_column(
        Integer,
        nullable=True,
        doc="Total experience across all skills"
    )
    ...
    def calculate_combat_level(self) -> Optional[int]:
        ...
        """
        required_skills = ['attack', 'strength', 'defence', 'hitpoints', 'prayer', 'ranged', 'magic']
        levels = {}
        ...
        base = 0.25 * (levels['defence'] + levels['hitpoints'] + levels['prayer'] // 2)
