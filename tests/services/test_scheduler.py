...
class TestPlayerScheduleManager:
    ...
    async def test_verify_all_schedules_success(
        ...
        """Test verification of all schedules."""
        ...
        result = await schedule_manager.verify_all_schedules()
    ...
    @pytest.mark.asyncio
    async def test_verify_all_schedules_with_issues(
        self, schedule_manager, mock_redis_source
    ):
        """Test verification of all schedules with various issues."""
        # Mock schedules with issues
        mock_schedule1 = MagicMock()
        mock_schedule1.schedule_id = "player_fetch_123"
        mock_schedule1.labels = {"player_id": "456"}  # Wrong player_id

        mock_schedule2 = MagicMock()
        mock_schedule2.schedule_id = "player_fetch_invalid"  # Invalid format
        mock_schedule2.labels = {"player_id": "789"}

        # Duplicate schedule
        mock_schedule3 = MagicMock()
        mock_schedule3.schedule_id = "player_fetch_123"  # Duplicate
        mock_schedule3.labels = {"player_id": "123"}
        ...
        assert result["total_schedules"] == 3
        assert result["player_fetch_schedules"] == 3
        assert (
            len(result["invalid_schedules"]) == 2
        )  # Wrong player_id and invalid format
        assert len(result["duplicate_schedules"]) == 1  # One duplicate
