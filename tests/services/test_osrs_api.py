...
class TestOSRSAPIClient:
    ...
    def mock_hiscore_response(self):
        ...
        return {
            "name": "TestPlayer",
            "skills": [
                {"id": 0, "name": "Overall", "rank": 1000, "level": 1500, "xp": 50000000},
                {"id": 1, "name": "Attack", "rank": 500, "level": 99, "xp": 13034431},
                {"id": 2, "name": "Defence", "rank": 600, "level": 90, "xp": 5346332},
                {"id": 3, "name": "Strength", "rank": 400, "level": 99, "xp": 13034431}
            ],
            "activities": [
                {"id": 0, "name": "Abyssal Sire", "rank": 100, "score": 50},
                {"id": 1, "name": "Zulrah", "rank": 200, "score": 100},
                {"id": 2, "name": "Vorkath", "rank": -1, "score": -1}  # No kills
            ]
        }
    ...
    async def test_parse_hiscore_data_complete(self, client, mock_hiscore_response):
        ...
        assert result.overall == {"rank": 1000, "level": 1500, "experience": 50000000}
        assert "attack" in result.skills
        assert result.skills["attack"] == {"rank": 500, "level": 99, "experience": 13034431}
    ...
    async def test_rate_limiting(self, client):
        ...
        with patch.object(client, '_session') as mock_session:
            ...
            assert time_diff >= client.RATE_LIMIT_DELAY - 0.1  # Allow small timing variance
    ...
    async def test_check_player_exists_found(self, client, mock_hiscore_response):
...
class TestOSRSAPIClientIntegration:
    ...
    async def test_fetch_nonexistent_player(self, client):
        ...
        with pytest.raises(PlayerNotFoundError):
            await client.fetch_player_hiscores("ThisPlayerShouldNotExist123456789")