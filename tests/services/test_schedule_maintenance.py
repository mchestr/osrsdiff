...
class TestScheduleMaintenanceService:
    ...
    def mock_schedule_manager(self):
        ...
        manager = MagicMock(spec=PlayerScheduleManager)
        manager.redis_source = AsyncMock()
        manager.ensure_player_scheduled = AsyncMock()
        manager._verify_schedule_exists_and_valid = AsyncMock(
            return_value=True
        )
        return manager

    @pytest.fixture
    def maintenance_service(self, mock_schedule_manager):
        """Create a maintenance service instance."""
        return ScheduleMaintenanceService(mock_schedule_manager)
    ...
    async def test_cleanup_orphaned_schedules_no_schedules(
        self, maintenance_service, test_session, mock_schedule_manager
    ):
        """Test cleanup when no schedules exist."""
        mock_schedule_manager.redis_source.get_schedules = AsyncMock(
            return_value=[]
        )
    ...
    async def test_cleanup_orphaned_schedules_dry_run(
        ...
        with patch("app.services.schedule_maintenance.select") as mock_select:
            ...
            mock_schedule_manager.redis_source.delete_schedule.assert_not_called()
    ...
    async def test_fix_player_schedule_success(
        ...
        mock_schedule_manager.ensure_player_scheduled = AsyncMock(
