...
class TestPlayerModel:
    ...
    def test_validate_username_valid_cases(self):
        ...
        for username in valid_usernames:
            assert Player.validate_username(username) is True, f"Username '{username}' should be valid"
    ...
    def test_validate_username_invalid_cases(self):
        ...
        for username in invalid_usernames:
            assert Player.validate_username(username) is False, f"Username '{username}' should be invalid"
...
class TestPlayerValidation:
    ...
    def test_username_special_characters(self):
        ...
        invalid_chars = ["test@user", "test.user", "test+user", "test/user", "test\\user"]
...
class TestPlayerDatabaseOperations:
    ...
    async def player_with_records(self, test_session: AsyncSession):
        ...
        record2 = HiscoreRecord(
            ...
            skills_data={"attack": {"rank": 400, "level": 99, "experience": 13034431}},
    ...
    @pytest.mark.asyncio
    async def test_query_player_by_username(self, test_session: AsyncSession, sample_player: Player):
    ...
    @pytest.mark.asyncio
    async def test_update_player_in_database(self, test_session: AsyncSession, sample_player: Player):
    ...
    @pytest.mark.asyncio
    async def test_delete_player_from_database(self, test_session: AsyncSession, sample_player: Player):
    ...
    @pytest.mark.asyncio
    async def test_player_hiscore_relationship(self, test_session: AsyncSession, player_with_records: Player):
    ...
    @pytest.mark.asyncio
    async def test_cascade_delete_hiscore_records(self, test_session: AsyncSession, player_with_records: Player):
        ...
        stmt = select(HiscoreRecord).where(HiscoreRecord.player_id == player_id)
        result = await test_session.execute(stmt)
    ...
    @pytest.mark.asyncio
    async def test_unique_username_constraint(self, test_session: AsyncSession):
    ...
    @pytest.mark.asyncio
    async def test_latest_hiscore_property_with_database(self, test_session: AsyncSession, player_with_records: Player):
        ...
        assert latest.overall_level in [1500, 1600]  # Either could be latest depending on timing